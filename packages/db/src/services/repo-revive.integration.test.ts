...
async function read() {
	const rows = await db
		.select()
		.from(repos)
		.where(and(eq(repos.forge, "gitlab"), eq(repos.fullName, FULL_NAME)));
	return rows[0];
}
...
test("re-import revives a disconnected repo and releases its old org", async () => {
	...
	expect(await countUnclaimedRepos(db, "gitlab", ACCOUNT)).toBe(0);
	...
	expect(await countUnclaimedRepos(db, "gitlab", ACCOUNT)).toBe(1);
});
...
test("disconnect drops the ownership row so re-imported repos stay claimable", async () => {
	...
	const result = await removeInstallation(db, "gitlab", "orphan-acct");
	...
	const claims = await db
		...
		.where(
			and(
				eq(organizationInstallations.forge, "gitlab"),
	...
	expect(await countUnclaimedRepos(db, "gitlab", "orphan-acct")).toBe(1);
});
